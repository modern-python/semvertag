...
class GitLabProvider:
    ...
    def list_tags(self) -> list[Tag]:
        return _rest.collect_link_pages(
            self.http,
            url=f"{_API_PREFIX}/{self.project_id}/repository/tags",
            params={"per_page": _TAGS_PER_PAGE, "page": 1},
            response_model=_TagList,
            extract=lambda page: [Tag(name=item.name, commit_sha=item.commit.id) for item in page.root],
            translate=lambda exc: _errors.translate_gitlab(exc, project_id=self.project_id),
